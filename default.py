...
def startStreamDownload():
	...
	if (len(vPath) > 0) and (len(pType) > 0) and (iIS==True):
		...
		###
		eod(); DoA("Back"); 
		DoDownloader(vUrl,vName,vPath,False); 
	else: eod(); DoA("Back"); 
...
def startStreamPlay():
	...
	if (len(vPath) > 0) and (len(pType) > 0) and (iIS==True):
		...
		###
		eod(); DoA("Back"); 
		DoPlayer(vUrl,vName,vPath,False); 
	else: eod(); DoA("Back"); 
...
def SectionMenu():
	...
	if (len(vPath) > 0) and (len(pType) > 0) and (iIS==True):
		...
		vName=setupName(vName,pType,vUrl); 
		###
		_addon.add_directory({'mode':'Play','url':vUrl,'destfile':vName,'filetype':pType,'destpath':vPath,'site':site,'section':section},{'title':cFL('Download '+str(pType)+': ','firebrick')+cFL(vName,'mediumpurple')+CR+vUrl},is_folder=True,fanart=fanartSite,img=iconSite)
	###
...
def mode_subcheck(mode='',site='',section='',url=''):
	...
	elif (mode=='AxelPlay'): startStreamPlay()
	elif (mode=='AxelDownload'): startStreamDownload()
	...
	## XBMC.RunPlugin(plugin://plugin.program.streamrecord/?mode=Record)  ## this one seems to work in skins.
		...
		DoPlayer(url,addpr('destfile',''),addpr('destpath',''),useResolver=False)
	elif (mode=='PlayR'):
		DoPlayer(url,addpr('destfile',''),addpr('destpath',''),useResolver=True)
	#elif (mode=='Download'):
